...
func (cfg *config) checkLogs(i int, m ApplyMsg) (string, bool) {
	...
	for j := 0; j < len(cfg.logs); j++ {
		if old, oldok := cfg.logs[j][m.CommandIndex]; oldok && old != v {
			log.Printf("%v: LogEntry %v; server %v\n", i, cfg.logs[i], cfg.logs[j])
			// some server has already committed a different value for this entry!
			err_msg = fmt.Sprintf("commit nextLogIndex=%v server=%v %v != server=%v %v",
...
// applier reads message from apply ch and checks that they match the LogEntry
// contents
	...
	for m := range applyCh {
		...
		if m.SnapshotValid {
			...
			if m.CommandIndex != cfg.lastApplied[i]+1 {
				err_msg = fmt.Sprintf("server %v apply out of order, expected nextLogIndex %v, got %v", i, cfg.lastApplied[i]+1, m.CommandIndex)
			}
...
func (cfg *config) checkOneLeader() int {
	for iters := 0; iters < 10; iters++ {
		...
		Debug(nil, dInfo, "sleep %v ms", ms)
		time.Sleep(time.Duration(ms) * time.Millisecond)
		Debug(nil, dInfo, "sleep donw")
...
func (cfg *config) checkNoLeader() {
	for i := 0; i < cfg.n; i++ {
		if cfg.connected[i] {
			...
			Debug(nil, dInfo, "%d %v %v\n", i, term, is_leader)
			if is_leader {
				Debug(nil, dError, "test false")
...
// how many servers think a LogEntry entry is committed?
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		if ok {
			if count > 0 && cmd != cmd1 {
				cfg.t.Fatalf("committed values do not match: nextLogIndex %v, %v, %v",
					index, cmd, cmd1)
			}
			count += 1
			cmd = cmd1
...
func (cfg *config) wait(index int, n int, startTerm int) interface{} {
	...
	if nd < n {
		cfg.t.Fatalf("only %d decided for nextLogIndex %d; wanted %d",
			nd, index, n)
	}
	return cmd
}
...
// Maximum LogEntry size across all servers
