...
func spawn_clients_and_wait(t *testing.T, cfg *config, ncli int, fn func(me int, ck *Clerk, t *testing.T)) {
	...
	// Debug(nil, dInfo"spawn_clients_and_wait: waiting for clients")
	for cli := 0; cli < ncli; cli++ {
		ok := <-ca[cli]
		// Debug(nil, dInfo"spawn_clients_and_wait: client %d is done\n", cli)
		if ok == false {
			t.Fatalf("failure")
		}
	}
...
func GenericTest(t *testing.T, part string, nclients int, nservers int, unreliable bool, crash bool, partitions bool, maxraftstate int, randomkeys bool) {
	...
	for i := 0; i < 3; i++ {
		...
		go spawn_clients_and_wait(t, cfg, nclients, func(cli int, myck *Clerk, t *testing.T) {
			...
			for atomic.LoadInt32(&done_clients) == 0 {
				...
				if (rand.Int() % 1000) < 500 {
					// Debug(nil, dInfo"%d: client new append %v\n", cli, nv)
					...
					// Debug(nil, dInfo"%d: client new get %v\n", cli, key)
		...
		if partitions {
			// Debug(nil, dInfo"wait for partitioner\n")
			<-ch_partitioner
		...
		if crash {
			Debug(nil, dInfo, "shutdown servers\n")
			...
			time.Sleep(electionTimeout)
			Debug(nil, dInfo, "restart servers\n")
		...
		Debug(nil, dInfo, "wait for clients\n")
		for i := 0; i < nclients; i++ {
			Debug(nil, dInfo, "read from clients %d\n", i)
			j := <-clnts[i]
			// if j < 10 {
			// 	Debug(nil, dInfo"Warning: client %d managed to perform only %d put operations in 1 sec?\n", i, j)
			// }
			key := strconv.Itoa(i)
			Debug(nil, dInfo, "Check %v for client %d\n", j, i)
